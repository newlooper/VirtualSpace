...
namespace VirtualSpace.AppLogs
{
    public static class LogManager
    {
        ...
        public static readonly Serilog.Core.Logger RootLogger = new LoggerConfiguration()
            ...
            .WriteTo.Logger( c =>
                c.Filter.ByIncludingOnly( evt => evt.Level == LogEventLevel.Debug )
                    .WriteTo.File( "Logs/event.txt", LogEventLevel.Debug ) )
            .WriteTo.Logger( c =>
                c.Filter.ByIncludingOnly( evt => evt.Level == LogEventLevel.Information )
                    .WriteTo.File( "Logs/info.txt", LogEventLevel.Information ) )
            .WriteTo.Logger( c =>
                c.Filter.ByIncludingOnly( evt => evt.Level == LogEventLevel.Warning )
                    .WriteTo.File( "Logs/warning.txt", LogEventLevel.Warning ) )
            .WriteTo.Logger( c =>
                c.Filter.ByIncludingOnly( evt => evt.Level == LogEventLevel.Error )
                    .WriteTo.File( "Logs/error.txt", LogEventLevel.Error ) )
            .WriteTo.Logger( c =>
                c.Filter.ByIncludingOnly( evt => evt.Level == LogEventLevel.Fatal )
                    .WriteTo.File( "Logs/fatal.txt", LogEventLevel.Fatal ) )
            .CreateLogger();
